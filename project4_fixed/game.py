...
class Game():
    def __init__(self, players, log=False):
        ...
        for i, cards in zip([0, 1], self.player_cards):
            ...
            if log:
                logging.info(
                    f"Player ({i + 1}): {self.players[i].name} received:")
                logging.info(self.players[i].cards)
    ...
    def takeTurn(self, log=False):
        ...
        activePlayer: Player = self.players[self.player_move]
        opponent: Player = self.players[1 - self.player_move]
        ...
        if decision == "draw":
            ...
            self.stats["draw_decisions"][self.player_move] += 1
